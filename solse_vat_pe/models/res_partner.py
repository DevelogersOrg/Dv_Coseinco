...
def getDatosDNI(self, dni, tipo_busqueda, token):
...
def get_data_doc_number(self, tipo_doc, numero_doc, tipo_busqueda, token, format='json'):
	ditrict_obj = self.env['l10n_pe.res.city.district']
	if tipo_doc == "dni" or tipo_doc=="01" or tipo_doc=="1":
		_logger.info("busqueda por dni")
		res = {'error': False, 'message': 'OK', 'data': {'success': True, 'data': getDatosDNI(self, numero_doc, tipo_busqueda, token)}}
		return res
...
class Partner(models.Model):
	...
	def consulta_datos(self, tipo_documento, nro_documento, format='json'):
		...
		try:
			res = get_data_doc_number(self, tipo_documento, str(nro_documento), tipo_busqueda, token, format='json')
		except Exception as e:
	...
	def consulta_datos_completo(self, tipo_documento, nro_documento, format='json'):
		...
		try:
			...
			res['message'] = 'Error en la conexion: '+str(empresa.name)
			return res
