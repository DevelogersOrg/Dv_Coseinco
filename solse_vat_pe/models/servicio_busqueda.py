...
def get_dni_apiperu(token, dni):
	...
	if datos_dni.status_code == 200:
		...
		if datos['success'] == False:
			ayuda = "\nSi desea registrar los datos de forma manual, desactive la casilla 'Busqueda automatica'"
			raise ValidationError(datos['message']+ayuda)
...
def get_ruc_apiperu(token, ruc):
	...
	if datos_ruc.status_code == 200:
		...
		if datos_ruc['success'] == False:
			ayuda = "\nSi desea registrar los datos de forma manual, desactive la casilla 'Busqueda automatica'"
			raise ValidationError(datos_ruc['message']+ayuda)
...
def get_ruc_apimigo(token, ruc):
	...
	try:
		...
		if datos_request.status_code == 200:
			...
			datos['error'] = False if datos['success'] else True
			return datos
