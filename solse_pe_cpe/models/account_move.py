...
class AccountMove(models.Model):
    ...
    pe_amount_discount = fields.Monetary(string='Descuento', compute='_compute_discount', tracking=True)
    pe_total_discount_tax = fields.Monetary(string='Impuesto de descuento', compute='_compute_discount', tracking=True)
    ...
    total_operaciones_gravadas = fields.Monetary(store=True, readonly=True,compute='_compute_amount_reporte', track_visibility='onchange')
    total_operaciones_exoneradas = fields.Monetary(store=True, readonly=True,compute='_compute_amount_reporte', track_visibility='onchange')
    total_operaciones_gratuitas = fields.Monetary(store=True, readonly=True, compute='_compute_amount_reporte', track_visibility='onchange')
    total_operaciones_inafectas = fields.Monetary(store=True, readonly=True,compute='_compute_amount_reporte', track_visibility='onchange')	
    total_operaciones_exportadas = fields.Monetary(store=True, readonly=True, compute='_compute_amount_reporte', track_visibility='onchange')
    total_otros_cargos = fields.Monetary()
    total_final_parcial = fields.Monetary(store=True, readonly=True, compute='_compute_amount_reporte', track_visibility='onchange')
    ...
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        ...
        if view_type in ['form']:
            ...
            if self._context.get('params') and 'action' in self._context['params']:
                ...
                if not accion or not accion.domain:
                    paso_validacion = True
                elif 'out_invoice' in accion.domain or 'out_refund' in accion.domain:
                    paso_validacion = True
    ...
    def _compute_amount_reporte(self,detener=False):	
        ...
        """
        for reg in self:
            reg.total_operaciones_gravadas = sum(line.price_subtotal for line in reg.invoice_line_ids if line.tax_ids and line.tax_ids[0].l10n_pe_edi_tax_code == '1000')	
            reg.total_operaciones_exoneradas = sum(line.price_subtotal for line in reg.invoice_line_ids if line.tax_ids and line.tax_ids[0].l10n_pe_edi_tax_code == '9997')
            #reg.total_operaciones_gratuitas = sum(line.price_subtotal_signed for line in reg.invoice_line_ids if line.nombre_impuesto == 'GRA')
            reg.total_operaciones_gratuitas = sum(line.price_subtotal * line.quantity for line in reg.invoice_line_ids if line.tax_ids and line.tax_ids[0].l10n_pe_edi_tax_code == '9996')
            reg.total_operaciones_inafectas = sum(line.price_subtotal for line in reg.invoice_line_ids if line.tax_ids and line.tax_ids[0].l10n_pe_edi_tax_code == '9998')
            reg.total_operaciones_exportadas = sum(line.price_subtotal for line in reg.invoice_line_ids if line.tax_ids and line.tax_ids[0].l10n_pe_edi_tax_code == '9995')

            reg.total_final_parcial = reg.amount_untaxed + reg.amount_tax
    ...
    def action_invoice_sent(self):
        ...
        if self.l10n_latam_document_type_id.is_cpe and self.pe_cpe_id:
            ...
            if 'default_attachment_ids' in res['context']:
                new_default_attachment_ids = res['context']['default_attachment_ids'] + default_attachment_ids
            else:
                new_default_attachment_ids = default_attachment_ids
            vals = {}
