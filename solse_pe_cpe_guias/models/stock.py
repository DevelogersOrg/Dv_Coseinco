...
class Picking(models.Model):
	...
	pe_unit_quantity = fields.Integer("Cantidad unitaria", copy=False)
	...
	modificar_fecha_entrega = fields.Boolean("Modificar fecha entrega", compute="_compute_almacen", store=True, help="Permite modificar la fecha de entrega despues de confirmado")
	...
	def _set_scheduled_date(self):
		for picking in self:
			if picking.state in ('done', 'cancel') and picking.modificar_fecha_entrega == False:
	...
	def _compute_almacen(self):
		for reg in self:
			...
			modificar_fecha_entrega = True
			if almacen_origen and almacen_origen.modificar_fecha_entrega:
				modificar_fecha_entrega = almacen_origen.modificar_fecha_entrega
			elif reg.state in ['done', 'cancel']:
				modificar_fecha_entrega = False

			reg.modificar_fecha_entrega = modificar_fecha_entrega
	...
	@api.model
	@api.depends('sale_id', 'sale_id.order_line', 'sale_id.order_line.invoice_lines')
...
class Warehouse(models.Model):
	...
	modificar_fecha_entrega = fields.Boolean("Modificar fecha entrega", help="Permite modificar la fecha de entrega despues de confirmado")
